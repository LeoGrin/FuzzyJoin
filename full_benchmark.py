...
if __name__ == "__main__":
    datasets = ["Country", "BasketballTeam",
                "Drug", "Device",
                "ArtificialSatellite",
                "Amphibian", "Song",
                "HistoricBuilding", "Wrestler",
                "EthnicGroup"]
    df = pd.DataFrame()
    for dataset in datasets:
                ...
                for similarity in ["cosine", "l1", "l2"]:
                    ...
                    df = df.append(
                        pd.DataFrame.from_dict(
                            {"dataset": [dataset] * n_points, "model": [model_name] * n_points,
        ...
        for similarity in ["cosine", "l1", "l2"]:
            ...
            df = df.append(
                pd.DataFrame.from_dict(
                    {"dataset": [dataset] * n_points, "model": [model_name] * n_points,
                     "precision": precision_ft, "recall": recall_ft, "f1": f1_ft},
                    orient="index").transpose(),
                ignore_index=True)
        ...
        for scorer in partial_ratio, ratio, WRatio:
            ...
            df = df.append(
                pd.DataFrame.from_dict(
                    {"dataset": [dataset] * n_points, "model": [model_name] * n_points,
                        "precision": precision_fw, "recall": recall_fw, "f1": f1_fw}, orient="index").transpose(),
                ignore_index=True)


        precision_fj, recall_fj, f1_fj = autofj_precision_recall(left_1, right_1, gt_1, n_points=10)
        model_name = "autofj_default"
        n_points = len(precision_fj)
        df = df.append(
            pd.DataFrame.from_dict(
                {"dataset": [dataset] * n_points, "model": [model_name] * n_points,
                    "precision": precision_fj, "recall": recall_fj, "f1": f1_fj}, orient="index").transpose(),
            ignore_index=True)
